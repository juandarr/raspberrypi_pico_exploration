...
static const char *NTP_HOSTS[] = {"pool.ntp.org", "time.google.com",
...
static const char *HTTP_TIME_PATH =
    "/api/timezone/America/Bogota.txt"; // returns lines with "unixtime: <n>"
...
static void timeSinceUpdate(long elapsedMs, uint32_t upds) {
  ...
  if (last_updated_hour != hours || last_updated_minute != minutes ||
    ...
    clearAndDrawText(timeBuffer, tft.textWidth("SinceUpd: "), UPDATE_Y, 126, 14,
                     TFT_GREENYELLOW, TFT_BLACK);
  ...
  if (last_updates != updates) {
    ...
    clearAndDrawText(updatesBuffer, tft.textWidth("Update #: "), UPDATE_Y + 35,
...
static bool ntpSend(IPAddress serverIp, uint32_t &sendStampMs) {
  ...
  if (ntpUDP.beginPacket(serverIp, NTP_PORT) != 1) {
...
static bool ntpTryReceive(uint32_t &unixOut, uint32_t sendStampMs) {
      uint8_t pkt[48];
      ...
      if (sz >= 48) {
        ...
        uint32_t secsSince1900 = ((uint32_t)pkt[40] << 24) | ((uint32_t)pkt[41]
      << 16) | ((uint32_t)pkt[42] << 8) | (uint32_t)pkt[43];
        ...
        Serial.print("[ntp] rx OK. round trip="); Serial.print(roundTripMs);
      Serial.println("ms");
        Serial.print("[ntp] server unix="); Serial.println(serverUnixTime);
        Serial.print("[ntp] correction="); Serial.print(correctionMs); Serial.
     println("ms");
        Serial.print("[ntp] corrected unix="); Serial.println(unixOut);
...
static bool httpGetUnixTime(uint32_t &unixOut) {
      ...
      Serial.print("[http] GET http://"); Serial.print(HTTP_TIME_HOST); Serial.
      println(HTTP_TIME_PATH);
      if (!client.connect(HTTP_TIME_HOST, HTTP_PORT)) {
        Serial.println("[http] connect failed");
        return false;
      }
   
      // Send a standards-compliant HTTP GET request
      client.print(String("GET ") + HTTP_TIME_PATH + " HTTP/1.1\r\n" +
                   "Host: " + HTTP_TIME_HOST + "\r\n" +
                   "Connection: close\r\n\r\n");
...
static void smEnter(SyncState s) {
  st = s;
  stSince = millis();
}
...
void loop() {
  ...
  case ST_IDLE: {
    ...
    timeUpdate();
    tempUpdate();
    timeSinceUpdate(now - lastSyncMillis, updates);
    break;
  }
  case ST_WIFI_WARMUP: {
    ntpHostIndex = 0;
  ...
  case ST_NTP_WAIT: {
    ...
    if (ntpTryReceive(unixUtc, ntpSendAt)) {
      ...
      ntpUDP.stop();
      ntpHostIndex++;
      if (ntpHostIndex >= NTP_HOST_COUNT){
